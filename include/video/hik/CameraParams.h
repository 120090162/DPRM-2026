...
#define MV_CAMERALINK_DEVICE \
  0x00000008  ///< \~chinese CamLink设备                  \~english CamLink Device
...
typedef struct _MV_GIGE_DEVICE_INFO_
{
  ...
  unsigned int
    nIpCfgCurrent;  ///< \~chinese       \~english IP configuration:bit31-static bit30-dhcp bit29-lla
...
{
  unsigned char
    CrtlInEndPoint;  ///< \~chinese 控制输入端点         \~english Control input endpoint
  unsigned char
    CrtlOutEndPoint;  ///< \~chinese 控制输出端点         \~english Control output endpoint
  ...
  unsigned char
    chVendorName[INFO_MAX_BUFFER_SIZE];  ///< \~chinese 供应商名字           \~english Vendor Name
  unsigned char
    chModelName[INFO_MAX_BUFFER_SIZE];  ///< \~chinese 型号名字             \~english Model Name
  unsigned char
    chFamilyName[INFO_MAX_BUFFER_SIZE];  ///< \~chinese 家族名字             \~english Family Name
  unsigned char chDeviceVersion
    [INFO_MAX_BUFFER_SIZE];  ///< \~chinese 设备版本号           \~english Device Version
  unsigned char chManufacturerName
    [INFO_MAX_BUFFER_SIZE];  ///< \~chinese 制造商名字           \~english Manufacturer Name
  unsigned char chSerialNumber
    [INFO_MAX_BUFFER_SIZE];  ///< \~chinese 序列号               \~english Serial Number
  unsigned char chUserDefinedName
    [INFO_MAX_BUFFER_SIZE];  ///< \~chinese 用户自定义名字       \~english User Defined Name
...
typedef struct _MV_CC_DEVICE_INFO_
{
  ...
  unsigned int
    nTLayerType;  ///< \~chinese 设备传输层协议类型，e.g. MV_GIGE_DEVICE\~english Device Transport Layer Protocol Type, e.g. MV_GIGE_DEVICE
...
{
  int64_t
    nReviceDataSize;  ///< \~chinese 已接收数据大小  [统计StartGrabbing和StopGrabbing之间的数据量]\~english Received Data Size  [Calculate the Data Size between StartGrabbing and StopGrabbing]
...
#define MV_MAX_TLS_NUM \
  8  ///< \~chinese 最多支持的传输层实例个数\~english The maximum number of supported transport layer instances
#define MV_MAX_DEVICE_NUM \
...
typedef struct _MV_CC_DEVICE_INFO_LIST_
{
  ...
  MV_CC_DEVICE_INFO * pDeviceInfo
    [MV_MAX_DEVICE_NUM];  ///< \~chinese 支持最多256个设备\~english Support up to 256 devices
...
typedef struct _MV_FRAME_OUT_
{
  ...
  MV_FRAME_OUT_INFO_EX
  stFrameInfo;  ///< \~chinese 图像信息\~english information of the specific image
...
typedef enum _MV_GIGE_TRANSMISSION_TYPE_ {
  ...
  MV_GIGE_TRANSTYPE_CAMERADEFINED =
    0x4,  ///< \~chinese 表示从相机获取，暂不支持\~english Transtype from camera,not support
...
{
  unsigned int
    nType;  ///< \~chinese 需要输出的信息类型，e.g. MV_MATCH_TYPE_NET_DETECT\~english Information type need to output ,e.g. MV_MATCH_TYPE_NET_DETECT
  void *
    pInfo;  ///< \~chinese 输出的信息缓存，由调用者分配\~english Output information cache, which is allocated by the caller
...
typedef struct _MV_IMAGE_BASIC_INFO_
{
  ...
  unsigned int
    nSupportedPixelFmtNum;  ///< \~chinese 支持的像素格式种类\~english Support pixel format
...
/// \~chinese Event事件回调信息\~english Event callback information
...
typedef struct _MV_XML_FEATURE_Value_
{
  ...
  char strDescription
    [MV_MAX_XML_DISC_STRLEN_C];  ///< \~chinese节点描述     目前暂不支持\~english Node Description, NOT SUPPORT NOW
  char strDisplayName[MV_MAX_XML_NODE_STRLEN_C];  ///< \~chinese 显示名称\~english Display Name
  char strName[MV_MAX_XML_NODE_STRLEN_C];         ///< \~chinese 节点名\~english Node Name
  char strToolTip[MV_MAX_XML_DISC_STRLEN_C];      ///< \~chinese 提示\~english Notice
  unsigned int nReserved[4];
...
typedef struct _MV_XML_FEATURE_Integer_
{
  ...
  char
    strDescription[MV_MAX_XML_DISC_STRLEN_C];  ///< \~chinese 目前暂不支持\~english NOT SUPPORT NOW
