#ifndef __DPRM_STRUCTURE_SPEEDQUEUE_HPP__
#define __DPRM_STRUCTURE_SPEEDQUEUE_HPP__
#include <deque>
#include <iostream>
#include <vector>
namespace rm {
...
class SpeedQueue {
public:
    SpeedQueue(): length_(3ull), init_((T)0) {
        deque_.resize(3ull, 0);
        percent_.resize(3ull, 1.0 / 3.0);
    }
    SpeedQueue(size_t length, T init): length_(length), init_(init) {
        deque_.resize(length_, init);
        percent_.resize(length_, 1.0 / (double)length_);
    }
    SpeedQueue(size_t length, T init, std::vector<double> pct): length_(length), init_(init) {
        ...
        for (int i = 0; i < length_; i++)
            percent_[length_ - i - 1] = pct[i] / pct_sum;
    }
    ~SpeedQueue() {}
    void push(T num) {
        T last = deque_.back();
