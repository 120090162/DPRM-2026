 ...
 */
#ifndef __DPRM_KALMAN_INTERFACE_TRACK_QUEUE_V1_H__
#define __DPRM_KALMAN_INTERFACE_TRACK_QUEUE_V1_H__
...
public:
    ...
    TQstateV1(): last_t(getTime()), count(0), exist(false), available(false) {
        model = new KF<6, 4>();
...
public:
    void setMinCount(int c) {
        this->min_count_ = c;
    }
    void setMaxDistance(double d) {
        this->max_distance_ = d;
    }
    void setMaxDelay(double d) {
        this->max_delay_ = d;
    }
    void setToggleAngleOffset(double d) {
        this->toggle_angle_offset_ = d;
    }
    void setMaxStd(double d) {
        this->max_std_ = d;
    }
...
private:
    double getAngleOffset(const Eigen::Matrix<double, 4, 1>& pose
    ); // 目标指向相对其与车身连线的夹角
    ...
    bool
    isDelayValid(const TimePoint& this_t, const TimePoint& last_t); // 根据更新延迟判断是否需要丢弃
