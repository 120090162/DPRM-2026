...
void getGrayScaleRGB(
    const cv::Mat& input,
    cv::Mat& gray,
    ArmorColor color
); // 通过RGB通道拆分获取灰度图
void getGrayScaleHSV(
    const cv::Mat& input,
    cv::Mat& gray,
    ArmorColor color
); // 通过HSV限制色相获取灰度图
void getGrayScaleCVT(const cv::Mat& input, cv::Mat& gray); // 将图像直接转换为灰度图
void getGrayScaleMix(
    const cv::Mat& input,
    cv::Mat& gray,
    ArmorColor color
); // 通过RGB和HSV混合方法获取灰度图
void getGrayScaleSub(
    const cv::Mat& input,
    cv::Mat& gray,
    ArmorColor color
); // 通过RGB通道差值方法获取灰度图
void getGrayScale(
    const cv::Mat& input,
    cv::Mat& gray,
    ArmorColor color = ARMOR_COLOR_BLUE,
    GrayScaleMethod method = GRAY_SCALE_METHOD_CVT
); // 获取灰度图统一接口

void getBinaryAverageThreshold(
    const cv::Mat& input,
    cv::Mat& binary,
    int threshold
); // 通过平均阈值获取二值图
void getBinaryMaxMinRatio(
    const cv::Mat& input,
    cv::Mat& binary,
    double ratio
); // 通过最大最小值比率获取二值图
void getBinary(
    ...
    cv::Mat& binary,
    double threshold,
    BinaryMethod method = BINARY_METHOD_MAX_MIN_RATIO
); // 获取二值图统一接口
...
ArmorColor
getArmorColorFromHSV(const cv::Mat& src, const rm::LightbarPair& rect); // 通过HSV获取装甲板颜色
ArmorColor
getArmorColorFromHSV(const cv::Mat& src, const rm::YoloRect& rect); // 通过HSV获取装甲板颜色
ArmorColor
getArmorColorFromRGB(const cv::Mat& src, const rm::LightbarPair& rect); // 通过RBG获取装甲板颜色
ArmorColor
getArmorColorFromRGB(const cv::Mat& src, const rm::YoloRect& rect); // 通过RBG获取装甲板颜色

double getLightHighHLS(const cv::Mat& src);

cv::Rect getExtendRect(
    const cv::Rect& rect,
    int src_width,
    int src_height,
    double ratio_x = 1.4,
...
ArmorColor
getRegionColor(const cv::Mat& src, const cv::RotatedRect& rotate_region); // 获取旋转矩形框内颜色
...
double getRatioAreaContourToRect(
...
double
getValueLengthLightbarPair(const Lightbar& lb1, const Lightbar& lb2); // 获取两个灯条的长度平均值
double getRatioLengthLightbarPair(
    const Lightbar& lb1,
    const Lightbar& lb2
); // 获取两个灯条的长度比, 大比小
double
getRatioAreaLightbarPair(const Lightbar& lb1, const Lightbar& lb2); // 获取两个灯条的面积比, 大比小
double
getRatioArmorSide(const Lightbar& lb1, const Lightbar& lb2); // 获取两灯条组成的可能装甲板的长宽比
double getAngleDiffLightbarPair(const Lightbar& lb1, const Lightbar& lb2); // 获取两个灯条的角度差
double
getAngleAvgLightbarPair(const Lightbar& lb1, const Lightbar& lb2); // 获取两个灯条的角度平均值
double getCenterOffsetLightbarPair(
    const Lightbar& lb1,
    const Lightbar& lb2
); // 获取两个灯条中心点的在平均方向上偏移量
...
cv::Point2f getRelativeToAbsoluteTrans(
    const cv::Rect& rect,
    const cv::Point2f& point
); // 将点坐标从iou矩形框的相对位置转换到原图的绝对位置

LightbarPair getBestMatchedLightbarPair(
    const std::vector<LightbarPair>& pairs,
    const Armor& armor
); // 获取最佳匹配的灯条对

bool getBestMatchedLightbarPair(
    const std::vector<rm::Lightbar>& lightbars,
    const Armor& armor,
...
void showHistogram(
    ...
    int wid = 0,
    int len = 0
); // 返回直方图的图像版，wid和len为直方图的宽和高
void setLine_Histogram(
    cv::Mat& input,
    cv::Mat& output,
    cv::Mat& histogram,
    int set_line = 0,
    int flag = 0
); // 在直方图的图像上画线, flag为0为水平线, 1为垂直线
void getHistogram(
    const cv::Mat& src,
    cv::Mat& histogram,
    int color = 0
); // 获取直方图，color默认为0灰色, 1为蓝色, 2为绿色, 3为红色
std::pair<int, int> getHistDoublePeak(const cv::Mat& histogram); // 获取直方图双峰值
void getHistIncludePeak(
    const cv::Mat& src,
    cv::Mat& ShowImage
); // 获取带有峰位置的直方图图像，竖线标识峰值位置
int getThresholdFromHist(const cv::Mat& src, int Cut_thresold, int bios = 0); // 通过直方图获取阈值
int getThresholdFromHist(
    const cv::Mat& src,
    cv::Mat& ShowImage,
    int Cut_thresold,
    int bios = 0
); // 通过自定义水平切割直方图 获取阈值，bios为向右偏移量
int getThresholdFromHistPeak(
    const cv::Mat& src,
    cv::Mat& ShowImage,
    int bios = 0
); // 通过直方图峰值获取阈值
...
PointPair findPointPairBarycenter(
    Lightbar lightbar,
    const cv::Mat& gray,
    double extend_dist = 32.0,
    double radius_ratio = 0.1
); // 重心法获取灯条轮廓
...
void setArmorExtendRectIOU(
    ...
    double ratio_x = 1.4,
    double ratio_y = 1.8
); // 从yolo扩展为装甲板IOU矩形框，具有边界检查

void setArmorBaseClass36(
    Armor& armor,
    const cv::Rect& rect,
    ...
    double ratio_x = 1.4,
    double ratio_y = 1.8,
    ArmorSize size = ARMOR_SIZE_SMALL_ARMOR
); // 设置装甲板基本信息

void setArmorBaseClass7(
    Armor& armor,
    const cv::Rect& rect,
    ArmorID armor_id,
    int src_width,
...
void setArmorFourPointsRelative(
    Armor& armor,
    const PointPair& pp0,
    const PointPair& pp1
); // 设置装甲板四个顶点相对位置
void setArmorFourPoints(
    Armor& armor,
    const PointPair& pp0,
    const PointPair& pp1
); // 设置装甲板四个顶点
...
void displaySingleArmorRect(
    const cv::Mat& src,
    const Armor& armor,
    int thickness = 1
); // 显示单个装甲板矩形框
void displaySingleArmorLine(
    const cv::Mat& src,
    const Armor& armor,
    int thickness = 1
); // 显示单个装甲板四个顶点
...
bool isRatioLengthLightbarPairValid(
    const Lightbar& lb1,
    const Lightbar& lb2,
    double max
); // 判断两个灯条长度比是否合法
bool isRatioAreaLightbarPairValid(
    const Lightbar& lb1,
    const Lightbar& lb2,
    double max
); // 判断两个灯条面积比是否合法，比较有用
bool isRatioArmorSideValid(
    const Lightbar& lb1,
    const Lightbar& lb2,
    double min,
    double max
); // 判断两个灯条组成的可能装甲板的长宽比是否合法
bool isAngleDiffLightbarPairValid(
    const Lightbar& lb1,
    const Lightbar& lb2,
    double max
); // 判断两个灯条角度差是否合法
bool isAngleAvgLightbarPairValid(
    const Lightbar& lb1,
    const Lightbar& lb2,
    double max
); // 判断两个灯条角度平均值是否合法
bool isCenterOffsetLightbarPairValid(
    const Lightbar& lb1,
    const Lightbar& lb2,
    double max
); // 判断归一化的偏移量是否合法，比较有用

bool isLightBarMatched(
    const Lightbar& lb1,
    const Lightbar& lb2,
    double max_ratio_length,
    double max_ratio_area,
...
bool isLightBarAreaPercentValid(
    Armor armor,
    double min_area_percent
); // 判断两个灯条围成的面积占装甲板面积是否过小
