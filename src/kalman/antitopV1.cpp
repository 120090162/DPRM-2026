...
AntitopV1::AntitopV1(double r_min, double r_max, int armor_num):
    r_min_(r_min),
    r_max_(r_max),
...
void AntitopV1::push(const Eigen::Matrix<double, 4, 1>& pose, const TimePoint& t, int toggle) {
    ...
    double v = sqrt(
        model_.estimate_X[4] * model_.estimate_X[4] + model_.estimate_X[5] * model_.estimate_X[5]
    );
...
void AntitopV1::setRadiusRange(double r_min, double r_max) {
    r_min_ = r_min;
    r_max_ = r_max;
}
void AntitopV1::setArmorNum(int armor_num) {
    armor_num_ = armor_num;
}
void AntitopV1::setStdValue(double std_v, double std_w) {
    fire_std_v_ = std_v;
    fire_std_w_ = std_w;
}
void AntitopV1::setFireValue(double angle, int fire_update) {
    fire_angle_ = angle;
    fire_update_ = static_cast<uint64_t>(fire_update);
}
...
double
AntitopV1::getAngleTrans(const double target_angle, const double src_angle, double refer_angle) {
    double dst_angle = src_angle;
...
bool AntitopV1::isFireValid(const Eigen::Matrix<double, 4, 1>& pose) {
    ...
    while (angle < -M_PI)
        angle += 2 * M_PI;
    if ((abs(angle) < fire_angle_) && (update_num_ > fire_update_))
        return true;
    return false;
}
