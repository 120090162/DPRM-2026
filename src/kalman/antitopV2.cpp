...
AntitopV2::AntitopV2(double r_min, double r_max, int armor_num):
    r_min_(r_min),
    r_max_(r_max),
...
void AntitopV2::setRadiusRange(double r_min, double r_max) {
    r_min_ = r_min;
    r_max_ = r_max;
}
void AntitopV2::setArmorNum(int armor_num) {
    armor_num_ = armor_num;
}
void AntitopV2::setFireValue(
    int update_num,
    double delay,
    double armor_angle,
    double center_angle
...
double
AntitopV2::getAngleTrans(const double target_angle, const double src_angle, double refer_angle) {
    double dst_angle = src_angle;
...
bool AntitopV2::getFireCenter(const Eigen::Matrix<double, 4, 1>& pose) {
    ...
    while (angle < -M_PI)
        angle += 2 * M_PI;
    if ((abs(angle) < fire_center_angle_) && (update_num_ > fire_update_))
        return true;
    return false;
}
