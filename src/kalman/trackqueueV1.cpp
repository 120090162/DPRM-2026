...
void TrackQueueV1::update() {
    ...
    if ((last_index_ == -1) || !list_[last_index_].exist) {
        ...
        return;
    } else if (last_index_ != min_angle_index && isCountUseValid(list_[min_angle_index].count)
               && isAngleOffsetValid(
...
void TrackQueueV1::setMatrixQ(double q1, double q2, double q3, double q4, double q5, double q6) {
    matrixQ_ << q1, 0, 0, 0, 0, 0, 0, q2, 0, 0, 0, 0, 0, 0, q3, 0, 0, 0, 0, 0, 0, q4, 0, 0, 0, 0, 0,
        0, q5, 0, 0, 0, 0, 0, 0, q6;
    for (int i = 0; i < list_.size(); i++) {
...
double TrackQueueV1::getDistance(
