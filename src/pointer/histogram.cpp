...
void rm::getHistogram(const cv::Mat& src, cv::Mat& histogram, int color) {
    ...
    switch (color) {
        case 0:
            histogram = Gray_hist.clone();
            break;

        case 1:
            histogram = B_hist.clone();
            break;

        case 2:
            histogram = G_hist.clone();
            break;

        case 3:
            histogram = R_hist.clone();
            break;
...
void rm::showHistogram(cv::Mat& histogram, cv::Mat& ShowImage, int wid, int len) {
    ...
    for (int i = 1; i < histSize; i++) {
        cv::line(
            histImage,
            cv::Point(bin_w * (i - 1), hist_h - cvRound(my_histogram.at<float>(i - 1))),
