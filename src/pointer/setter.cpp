...
void rm::setLigntbarAngleRectCV45(Lightbar& lightbar) {
    lightbar.angle =
        ((lightbar.rect.size.width > lightbar.rect.size.height) ? (lightbar.rect.angle - 90)
                                                                : (lightbar.rect.angle));
}
...
void rm::setLightbarRectBasedLength(Lightbar& lightbar) {
    lightbar.length =
        ((lightbar.rect.size.width > lightbar.rect.size.height) ? (lightbar.rect.size.width)
                                                                : (lightbar.rect.size.height));
}
    ...
    for (int i = 0; i < 4; i++) {
        ...
        for (int y = y_min; y <= y_max; ++y) {
            for (int x = x_min; x <= x_max; ++x) {
                if (cv::norm(cv::Point(x, y) - center) > r)
                    continue;
...
void rm::setArmorSizeByPoints(Armor& armor, double ratio) {
    if (ratio < 1.0)
        ratio = 1.0 / ratio;
    double width = cv::norm(armor.four_points[0] - armor.four_points[1])
        + cv::norm(armor.four_points[2] - armor.four_points[3]);
    double height = cv::norm(armor.four_points[0] - armor.four_points[2])
        + cv::norm(armor.four_points[1] - armor.four_points[3]);
