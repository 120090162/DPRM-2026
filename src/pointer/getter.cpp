...
double rm::getRatioRectSide(const cv::Rect& rect) {
    return (rect.width > rect.height) ? (double)(rect.width / rect.height)
                                      : (double)(rect.height / rect.width);
}

// 获取旋转矩形的长宽比，较长的边除以较短的边
double rm::getRatioRectSide(const cv::RotatedRect& rect) {
    return (rect.size.width > rect.size.height) ? (double)(rect.size.width / rect.size.height)
                                                : (double)(rect.size.height / rect.size.width);
}

// 获取轮廓的面积与轮廓最小外接矩形的面积的比值
double
rm::getRatioAreaContourToRect(const std::vector<cv::Point>& contour, const cv::RotatedRect& rect) {
...
void rm::getLightbarsFromContours(
...
double rm::getRatioLengthLightbarPair(const Lightbar& lightbar1, const Lightbar& lightbar2) {
    double ratio = (lightbar1.length > lightbar2.length) ? (lightbar1.length / lightbar2.length)
                                                         : (lightbar2.length / lightbar1.length);
    return ratio;
}
...
cv::Point2f rm::getArmorRectCenter(const Armor& armor) {
    return cv::Point2f(armor.rect.x + armor.rect.width / 2, armor.rect.y + armor.rect.height / 2);
}

// 将装甲板四点坐标从矩形框的相对坐标转换为图像的绝对坐标
cv::Point2f
rm::getRelativeToAbsoluteTrans(const cv::Rect& rect, const cv::Point2f& point_relative) {
    return cv::Point2f(rect.x + point_relative.x, rect.y + point_relative.y);
}

// 从匹配成对的灯条中获取最佳的一对
LightbarPair
rm::getBestMatchedLightbarPair(const std::vector<LightbarPair>& pairs, const Armor& armor) {
...
// 将灯条匹配成对，并筛选最佳配对
bool rm::getBestMatchedLightbarPair(
