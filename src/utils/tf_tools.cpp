...
void rm::tf_Mat3d(const cv::Mat& input_mat, Eigen::Matrix<double, 3, 3>& output_eigen) {
    output_eigen << input_mat.at<double>(0, 0), input_mat.at<double>(0, 1),
        input_mat.at<double>(0, 2), input_mat.at<double>(1, 0), input_mat.at<double>(1, 1),
        input_mat.at<double>(1, 2), input_mat.at<double>(2, 0), input_mat.at<double>(2, 1),
        input_mat.at<double>(2, 2);
}
...
void rm::tf_Vec4d(const cv::Mat& input_mat, Eigen::Matrix<double, 4, 1>& output_eigen) {
    output_eigen << input_mat.at<double>(0, 0), input_mat.at<double>(1, 0),
        input_mat.at<double>(2, 0), 1;
}

void rm::tf_Vec4f(const cv::Mat& input_mat, Eigen::Matrix<double, 4, 1>& output_eigen) {
    output_eigen << input_mat.at<float>(0, 0), input_mat.at<float>(1, 0), input_mat.at<float>(2, 0),
        1;
}
...
void rm::tf_pose2trans(
    const Eigen::Matrix<double, 4, 1>& input_pose,
    Eigen::Matrix<double, 4, 4>& output_matrix
) {
    output_matrix << 1, 0, 0, input_pose(0), 0, 1, 0, input_pose(1), 0, 0, 1, input_pose(2), 0, 0,
        0, 1;
}
...
void rm::tf_rt2trans(
    ...
    output_matrix << input_rotate(0, 0), input_rotate(0, 1), input_rotate(0, 2), input_pose(0),
        ...
        input_rotate(2, 0), input_rotate(2, 1), input_rotate(2, 2), input_pose(2), 0, 0, 0, 1;
}
...
void rm::tf_quater2trans(
    ...
    double m21 = 2 * qy * qz + 2 * qx * qw;
    double m22 = 1 - 2 * qx * qx - 2 * qy * qy;
    matrix << m00, m01, m02, 0, m10, m11, m12, 0, m20, m21, m22, 0, 0, 0, 0, 1;
}
