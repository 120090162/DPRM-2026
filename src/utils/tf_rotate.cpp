...
void rm::tf_rotate_pnp2cam(Eigen::Matrix<double, 3, 3>& matrix_rotate) {
    matrix_rotate << 0, 0, 1, -1, 0, 0, 0, -1, 0;
}
...
void rm::tf_rotate_pnp2head(
    ...
    rotate_cam_yaw << cos(cam_yaw), -sin(cam_yaw), 0, sin(cam_yaw), cos(cam_yaw), 0, 0, 0, 1;

    rotate_cam_pitch << cos(cam_pitch), 0, -sin(cam_pitch), 0, 1, 0, sin(cam_pitch), 0,
        cos(cam_pitch);

    rotate_cam_roll << 1, 0, 0, 0, cos(cam_roll), -sin(cam_roll), 0, sin(cam_roll), cos(cam_roll);

    rotate_flip << 0, 0, 1, -1, 0, 0, 0, -1, 0;
...
void rm::tf_rotate_head2world(
    ...
    rotate_yaw << cos(yaw), -sin(yaw), 0, sin(yaw), cos(yaw), 0, 0, 0, 1;

    rotate_pitch << cos(pitch), 0, -sin(pitch), 0, 1, 0, sin(pitch), 0, cos(pitch);
...
void rm::tf_rotate_head2world(
    ...
    rotate_roll << 1, 0, 0, 0, cos(roll), -sin(roll), 0, sin(roll), cos(roll);
...
void rm::tf_rotate_yaw2matrix(Eigen::Matrix<double, 3, 3>& matrix_rotate, const double armor_yaw) {
    double armor_pitch = -M_PI / 12.0;
    Eigen::Matrix<double, 3, 3> matrix_armor_pitch, matrix_armor_yaw;

    matrix_armor_pitch << cos(armor_pitch), 0, -sin(armor_pitch), 0, 1, 0, sin(armor_pitch), 0,
        cos(armor_pitch);

    matrix_armor_yaw << cos(armor_yaw), -sin(armor_yaw), 0, sin(armor_yaw), cos(armor_yaw), 0, 0, 0,
        1;
