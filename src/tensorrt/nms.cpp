...
static cv::Rect nms_get_rect(yolofpRaw* yolo_raw) {
    float min_x = std::min(
        std::min(yolo_raw->pose[0], yolo_raw->pose[2]),
        std::min(yolo_raw->pose[4], yolo_raw->pose[6])
    );
    float max_x = std::max(
        std::max(yolo_raw->pose[0], yolo_raw->pose[2]),
        std::max(yolo_raw->pose[4], yolo_raw->pose[6])
    );
    float min_y = std::min(
        std::min(yolo_raw->pose[1], yolo_raw->pose[3]),
        std::min(yolo_raw->pose[5], yolo_raw->pose[7])
    );
    float max_y = std::max(
        std::max(yolo_raw->pose[1], yolo_raw->pose[3]),
        std::max(yolo_raw->pose[5], yolo_raw->pose[7])
    );
...
static void nms_select_confidence_fp(std::vector<YoloRect>& list) {
    ...
    for (int i = 0; i < output_bboxes_num; i++) {
        ...
        if (iou_confidence < confidence_threshold)
            continue;
        ...
        for (int i = 0; i < 4; i++) {
            if (yolo_raw->pose[2 * i] < 1e-3 || yolo_raw->pose[2 * i] > (infer_width - 1.001)
                || yolo_raw->pose[2 * i + 1] < 1e-3
                || yolo_raw->pose[2 * i + 1] > (infer_height - 1.001))
            {
                flag_pose = false;
                break;
            }
        ...
        // 将推理框推入vector中
        if (detection_rect.four_points.size() != 4)
            continue;
...
static void nms_sort_confidence(std::vector<YoloRect>& list) {
    if (list.size() <= 1)
        return;
    std::sort(list.begin(), list.end(), [](const YoloRect& a, const YoloRect& b) {
...
static void nms_select_iou(std::vector<YoloRect>& list) {
    ...
    for (size_t focus_index = 1; focus_index < list.size(); focus_index++) {
        ...
        for (size_t retained_index = 0; retained_index < retained_list.size(); retained_index++) {
            // 计算当前遍历到的保留框和关注框的iou
            float iou = nms_calcu_iou(list[focus_index].box, retained_list[retained_index].box);
